...
def process_single_trace(
    ...
    try:
        ...
        if freq_min is not None and freq_max is not None:
            ...
            freq_min = max(freq_min, 1.0 / tr.times()[-1])
            tr.filter(
                "bandpass",
                freqmin=freq_min,
                freqmax=freq_max,
            ...
            logging.warning(
                f"Skipping filter for {tr.id}: no frequency limits provided"
            )
...
def batch_process(
    ...
    for idx in np.where(abs(rates - avg_rate) > 0.01)[0][::-1]:
        ...
        print(
            f"remove {stream[idx].stats.station} trace with sampling_rate deviate >0.01 Hz"
        )
