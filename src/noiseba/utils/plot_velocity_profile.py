...
def plot_velocity_profile(velocities, thickness, axes=None, unit="km", stride: int = 1, plot_kwargs=None):
    r"""
    ...
    # Initialize depth_intervals to avoid UnboundLocalError
    ...
    for i in range(0, len(velocities), stride):
        ...
        depth_intervals = np.cumsum(np.r_[0, model_thickness[:-1], 0.6 * model_thickness[-1]])
        ...
        for ind in range(len(model_velocities)):
            ...
            if ind == 0 and "label" in default_kwargs:
                ax.plot(
                    [model_velocities[ind], model_velocities[ind]],
            ...
            if ind < len(model_velocities) - 1:
                ax.plot(
                    [model_velocities[ind], model_velocities[ind + 1]],
